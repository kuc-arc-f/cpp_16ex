...
public:
    ...
    std::string getArraySearch(std::vector<float> embedding) {
        ...
        try {
            ...
            for (const auto& item : result_items) {
                if (vec_dim.size() < 3) {
                    ...
                    vec_dim.push_back(item.content);
                }        
            }
            std::cout << "vec_dim.size=" << vec_dim.size() << std::endl;
            json j1 = vec_dim; // 構造体を代入するだけ！
